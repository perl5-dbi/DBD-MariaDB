...
ok(!utf8::is_utf8($ref->[0]), "utf8 flag is not set without mariadb_enable_utf8mb4");

use utf8;
$dbh->{mariadb_enable_utf8mb4} = 1;
$sth = $dbh->prepare($query) or die "$DBI::errstr";
ok $sth->execute;
ok($ref = $sth->fetchrow_arrayref, 'fetch row with mariadb_enable_utf8mb4');
ok($sth->finish, 'close sth');
cmp_ok $ref->[0], 'eq', "😈", 'test U+1F608 with mariadb_enable_utf8mb4 and utf8 pragma';
cmp_ok $ref->[1], 'eq', "F09F9888";
$dbh->{mariadb_enable_utf8mb4} = 0;
no utf8;
