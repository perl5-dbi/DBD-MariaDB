...
DBD::MariaDB, the Perl DBI driver for the MySQL database. Before
...
use a binary distribution of DBD::MariaDB. If possible, this is
...
DBD::MariaDB is a DBI driver, hence you need DBI. It is available
from the same source where you got the DBD::MariaDB distribution
from.

=item C compiler

A C compiler is only required if you install from source. In
most cases there are binary distributions of DBD::MariaDB
...
Strawberry Perl comes bundled with DBD::MariaDB and the needed
client libraries.

=head3 ActiveState Perl

ActivePerl offers a PPM archive of DBD::MariaDB. All you need to
do is typing in a cmd.exe window:

  ppm install DBD-MariaDB
...
CentOS, and Fedora come with MySQL and DBD::MariaDB.

Use the following command to install DBD::MariaDB:
...
On Debian, Ubuntu and derivatives you can install DBD::MariaDB from
...
can install DBD::MariaDB from the repositories with the following
command:

    zypper install perl-DBD-MariaDB
...
The DBD::MariaDB Makefile.PL needs to know where to find your MySQL
...
As stated, to compile DBD::MariaDB you'll need a C compiler. This should
...
DBD::MariaDB installation. You can set any or all of the options, and
...
For a manual installation you need to fetch the DBD::MariaDB
source distribution. The latest version is always available
from

  https://metacpan.org/module/DBD::MariaDB

The name is typically something like

  DBD-MariaDB-4.025.tar.gz

The archive needs to be extracted. On Windows you may use a tool
like 7-zip, on *nix you type

  tar xf DBD-MariaDB-4.025.tar.gz

This will create a subdirectory DBD-MariaDB-4.025. Enter this
...
Port number of the test database; ignored when L</testhost> is set to
C<localhost>.

=item testsocket

Unix socket of the test database; takes effect only when L</testhost>
...
while installing DBD::MariaDB. I will only give a rough overview,
...
database licensed LGPL 2.1. To build DBD::MariaDB against this client, you
...
Once the client is built and installed, you can build DBD::MariaDB against
it:
...
But to install DBD::MariaDB you have to take a little special action.
...
download and extract DBD-MariaDB-<version>.tar.gz from CPAN

cd into unpacked dir DBD-MariaDB-<version>
you probably did that already, if you are reading this!

  cp /usr/local/mysql/bin/mysql_config .

This copies the executable script mentioned in the DBD::MariaDB docs
